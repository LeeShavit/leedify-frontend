...
async function getVideoId(currentSong) {
    ...
    try {
        const res = await axios.get(`https://www.googleapis.com/youtube/v3/search?part=snippet&q=${songName}&type=video&maxResults=1&key=${API_KEY}`)
        const ytId = res.data.items[0].id.videoId
        
        ytIdsMap[songName] = ytId
        saveToStorage(STORAGE_KEY, ytIdsMap)
        return ytId
    } catch (err) {
