...
export function Player() {
    ...
    function handleVolumeClick ({target}){
      ...
      async function onLikeDislikeSong() {
        try {
          const likedSongs = (!likedSongsIds.includes(currentSong._id)) ? await likeSong(currentSong) : await dislikeSong(currentSong._id)
    ...
    return (
        <section className="player full">
            <div className='song-info'>
                ...
                <button onClick={() => onLikeDislikeSong()} className={`like-song ${likedSongsIds.includes(currentSong._id) ? 'liked' : ''}`}>
